...
const validEnv = {
  ...
  REFRESH_TOKEN_SECRET: "test-refresh-token-secret-min-32-chars",
...
beforeEach(() => {
  ...
  vi.stubEnv("ACCESS_TOKEN_SECRET", validEnv.ACCESS_TOKEN_SECRET);
  vi.stubEnv("REFRESH_TOKEN_SECRET", validEnv.REFRESH_TOKEN_SECRET);
  vi.stubEnv("ADMIN_EMAIL", validEnv.ADMIN_EMAIL);
  vi.stubEnv("ADMIN_PASSWORD", validEnv.ADMIN_PASSWORD);
});
